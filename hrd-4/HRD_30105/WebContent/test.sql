...
select a.id, a.name, a.credit, b.name lecturer, a.week, a.start_hour, a.end_end from course_tbl a, lecturer_tbl b where a.lecturer = b.idx order by a.id;
select count(id) num from course_tbl;
...
update lecturer_tbl set name=?, major=?, field=? where idx=?