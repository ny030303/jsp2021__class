...
select max(custno) custno from member_tbl_02;
update MEMBER_TBL_02 set custname=?, phone=?, address=?, grade=?, city=? where custno=?
select custno, custname, phone, address, to_char(joindate, '"YYYY년 MM월 DD일"'), grade, city from member_tbl_02;
...
create table money_tbl_02 (
	...
	pcode		varchar2(4),
	sdate		date,
	constraint pk primary key(custno, salenol)
);

select * from money_tbl_02;

select a.custno, b.custname, grade, sum(a.price) price from money_tbl_02 a, member_tbl_02 b where a.custno = b.custno group by a.custno, b.custname order by a.custno;
